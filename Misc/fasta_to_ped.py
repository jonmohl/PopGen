...
for i in range(0,len(fasta_list[1][1])):
   ...
   for fasta in fasta_list:
      if fasta[1][i] not in nuc_list.keys() and fasta[1][i] != '?' and fasta[1][i] != 'N':
         nuc_list[fasta[1][i]] = 1
      elif fasta[1][i] != '?' and fasta[1][i] != 'N':
         nuc_list[fasta[1][i]] += 1
      if fasta[2][i] not in nuc_list.keys() and fasta[2][i] != '?' and fasta[2][i] != 'N':
         nuc_list[fasta[1][i]] = 1
      elif fasta[2][i] != '?' and fasta[2][i] != 'N':
...
for i in range(0,len(fasta_list[0][1])):
   if to_ped[i] == 1:
      ...
      for x in range(0,len(fasta_list)):
         if fasta_list[x][1][i] == '?' or fasta_list[x][1][i] == 'N' or fasta_list[x][2][i] == '?' or fasta_list[x][2][i] == 'N':
            ped[x] = ped[x] + '\t0\t0'
         else:
